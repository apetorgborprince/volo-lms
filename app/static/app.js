...
async function addStudent(){
 ...
 const username=prompt("Student username"); if(!username)return;
 const password=prompt("Temporary password (minimum 12 characters)"); if(!password || password.length<12){toast("Use a password of at least 12 characters.");return;}
 const cls=prompt("Class"); if(cls===null)return;
 try{await api("/api/students",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:name,username,password,class_name:cls})});loadStudents();toast("Student account created.");}
 catch(e){toast(e.message)}
}
async function addTeacher(){
 const name=prompt("Teacher full name"); if(!name)return;
 const username=prompt("Teacher username"); if(!username)return;
 const password=prompt("Temporary password (minimum 12 characters)"); if(!password || password.length<12){toast("Use a password of at least 12 characters.");return;}
 const subject=prompt("Subject focus","General Science"); if(subject===null)return;
 try{await api("/api/teachers",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({full_name:name,username,password,subject_focus:subject})});toast("Teacher account created.");}
 catch(e){toast(e.message)}
}
